...
public class MyElasticSearchDruidDataSource extends ElasticSearchDruidDataSource {

    private final static Log LOG = LogFactory.getLog(MyElasticSearchDruidDataSource.class);
    ...
    public MyElasticSearchDruidDataSource() {
        this(false);
    }

    public MyElasticSearchDruidDataSource(boolean fairLock) {
        super(fairLock);
    ...
    public void init(Settings settings) throws SQLException {
        if (inited) {
            return;
        }
        ...
        try {
            ...
            try {
                ...
                for (int i = 0, size = getInitialSize(); i < size; ++i) {
                    Connection conn = createPhysicalConnection(settings);
    ...
    public Connection createPhysicalConnection(Settings settings) throws SQLException {
        ...
        try {
            conn = createPhysicalConnection(url, connectProperties, settings);
    ...
    public Connection createPhysicalConnection(String url, Properties info , Settings settings) throws SQLException {
        Connection conn = new MyElasticSearchConnection(url, settings);
    ...
    public DruidPooledConnection getConnection(Settings settings) throws SQLException {
        return getConnection(maxWait, settings);
    }

    public DruidPooledConnection getConnection(long maxWaitMillis, Settings settings) throws SQLException {
        init(settings);
    ...
    public void registerMbean() {
        if (!mbeanRegistered) {
            AccessController.doPrivileged(new PrivilegedAction<Object>() {
                ...
                @Override
                public Object run() {
                    ObjectName objectName = DruidDataSourceStatManager.addDataSource(MyElasticSearchDruidDataSource.this,
                            MyElasticSearchDruidDataSource.this.name);

                    MyElasticSearchDruidDataSource.this.setObjectName(objectName);
                    MyElasticSearchDruidDataSource.this.mbeanRegistered = true;
    ...
    public void unregisterMbean() {
        if (mbeanRegistered) {
            AccessController.doPrivileged(new PrivilegedAction<Object>() {
                ...
                @Override
                public Object run() {
                    DruidDataSourceStatManager.removeDataSource(MyElasticSearchDruidDataSource.this);
                    MyElasticSearchDruidDataSource.this.mbeanRegistered = false;
                    return null;
                }
            });
        }
...
*/
    ...
    protected void put(Connection connection) {
        ...
        try {
            holder = new DruidConnectionHolder(MyElasticSearchDruidDataSource.this, connection);
        } catch (SQLException ex) {
            lock.lock();
            try {
                if (createScheduler != null) {
